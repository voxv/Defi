class PreScene extends Phaser.Scene {
  ...
  startGameSequence() {
	 if (!this.battleSoundPlayed) {
	 	...
	 	sound.on('complete', function() {
			g.sound.stopAll()
			socket.send(JSON.stringify({ type: 'finishedStartSequence', id: myid }))
		})
  ...
  addAvatar(options) {
	if (myid=='player1') {
		...
		this.joinButton.addEventListener("click",function () { socket.send(JSON.stringify({ type: 'startGameSequence' })) })
